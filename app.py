...
from reportlab.lib.units import cm      # <-- aquí sí viene cm
...
app.permanent_session_lifetime = timedelta(minutes=30)  # <-- agregado

# Configuración de MySQL
# ==================== CONFIGURACIÓN MYSQL (RENDER + RAILWAY) ====================
...
app.config['MYSQL_SSL'] = {'ssl': {'rejectUnauthorized': False}}
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            session.permanent = True  # <-- agregado
